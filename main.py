...
from pruning import MLP, compute_stats, copy_weights_mlp, prune_mlp, reinit_mlp
    ...
    for X_batch, Y_batch in iterable:
        ...
        if it % val_freq == 0 and dataloader_val is not None:
            ...
            for X_batch_val, y_batch_val in dataloader_val:
                is_equal.append(model(X_batch_val).argmax(dim=-1) == y_batch_val)
...
def main(argv=None):
    ...
    dataset_train = MNISTDataset("data", train=True, download=True)
    dataset_val = MNISTDataset("data", train=False, download=True)
    ...
    if args.prune_ratio > 0:
        per_round_prune_ratio = 1 - (1 - args.prune_ratio) ** (1 / args.prune_iter)
